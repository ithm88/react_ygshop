...
class Index extends Component {
    ...
    handleCodeSend = (params) => {
        ...
        if (is_valid) {
            ...
            console.log("输入号码有效！");
            if (!this.timeId) {
                axios.get("http://115.29.148.10:8989/user/findRedis?telephone=" + this.state.phone).then(res => {
                    console.log(res)
    ...
    // 注册
    handleRegister=(e) => {
        let telephone = this.state.phone;
        let validateCode = this.state.code;
        let password = this.state.password;
        if(!telephone||!validateCode||!password){
        ...
        let url = "http://115.29.148.10:8989/login/zhuce?telephone="+telephone+"&&validateCode="+validateCode+"&&password="+password;
